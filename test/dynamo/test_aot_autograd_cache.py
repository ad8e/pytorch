...
def _opaque_custom_autograd_backward(grad):
    return grad * 2
...
class AOTAutogradCacheTests(CacheKeyEquivalenceMixin, InductorTestCase):
    ...
    def test_custom_autograd_function_cache_invalidation(self, changed_part):
        ...
        # A distinct but equivalent class still hits: class identity is not
        # part of the cache key, only the traced forward/backward code is.
        autograd_function_cls = make_autograd_function(use_cos=False, grad_scale=2)
        self._clear_dynamo_and_codecache()
        counters.clear()
        eager_x = torch.randn(5, device="cpu", requires_grad=True)
...
class AOTAutogradCachePicklerTests(torch._dynamo.test_case.TestCase):
    ...
    def test_custom_autograd_function_with_incompatible_nested_function(self):
        class MyAutogradFunction(torch.autograd.Function):
            ...
            @staticmethod
            def backward(ctx, grad_output):
                return _opaque_custom_autograd_backward(grad_output)
        ...
        with self.assertRaisesRegex(
            BypassAOTAutogradCache,
            r"Unsupported call_function target .*_opaque_custom_autograd_backward",
        ):
